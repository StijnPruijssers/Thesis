from models_us import get_us_cnn_simple
import os
...
run_params = {'dataset_name': 'us_rect_checked_hard',
              'hyper_param_name': 'US_exp1_3',
              'max_trials': 1000,
              'exec_per_trial': 1,
              'overwrite': False,
...
class myHypermodel(kt.HyperModel):
    def __init__(self, name=None, tunable=True, class_weights=[1, 1, 1, 1], x_size=(151, 76)):
        self.name = name
    ...
    def build(self, hp):
        ...
        gamma = hp.Fixed('gamma', value=0)
        opt = hp.Choice('opt', ['adam', 'sgdm'])
        dense_units = hp.Choice('dense_units', [64, 128, 256])

        model = get_us_cnn_simple(input_shape=(self.x_size[0], self.x_size[1], 1),
    ...
    def run_trial(self, trial, *fit_args, **fit_kwargs):
        ...
        for i, (train_fold, val_fold) in enumerate(skf.split(data['train_ids'], data['labels_stratify'])):
            ...
            wandb.setup(wandb.Settings(
                program='Z:/Personal folder/Stijn Pruijssers (M3)_UT_2021/Software/Code/CRNN_git3/us_exp1.py',
                program_relpath='us_exp1.py', mode=params['wandb_mode']))
            ...
            copied_fit_kwargs['callbacks'].append(
                WandbClassificationCallback(training_data=(x_train, train_labels_fold),
                                            ...
                                            layers=1))
            # Create checkpoints
            copied_fit_kwargs['callbacks'].append(EarlyStopping(monitor='val_loss',
                                                                mode='min',
                                                                verbose=1,
                                                                patience=150))
        ...
        self.oracle.update_trial(trial.trial_id, {'val_mcc': np.mean(mcc_tot)})
...
tuner.search(verbose=1, shuffle=True, data=data, params=run_params, epochs=run_params['epochs'])
